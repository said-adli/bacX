...
export async function checkRateLimitDistributed(
    ...
    // 1. Fail-Closed in Production if Upstash is missing
    if (!limiter && process.env.NODE_ENV === 'production' && !process.env.SKIP_RATE_LIMIT_CHECK) {
        console.error('[SECURITY CRITICAL] Rate Limiting failed closed: Upstash Redis not configured in production.');
        return {
            success: false,
            remaining: 0,
