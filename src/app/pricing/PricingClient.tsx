...
export default function PricingClient({ plans }: PricingClientProps) {
    ...
    // Map DB plans to UI format, or just use them directly. 
    // We'll augment them with some static UI defaults if needed (like 'popular' tag logic).
    // For now, let's map them to a uniform structure merging static 'free' plan + DB plans.

    const dbPlansFormatted = plans.map(p => ({
        id: p.id,
        name: p.name,
        price: p.discount_price ? p.discount_price.toLocaleString() : p.price.toLocaleString(),
        originalPrice: p.discount_price ? p.price.toLocaleString() : null,
        period: "/ سنة",
        description: p.description || "باقة شاملة",
        features: p.features,
        notIncluded: [],
        cta: "اشترك الآن",
        href: "/auth/signup",
        popular: p.name.includes("VIP") || p.price > 0, // Simple heuristic or could be a flag in DB
        isCustom: false
    }));
                            ...
                            <div className={cn(
                                ...
                                <div className="mb-8">
                                    ...
                                    <div className="flex items-baseline gap-2 mb-4">
                                        ...
                                        {tier.originalPrice && <span className="text-sm text-white/40 line-through decoration-red-500/50">{tier.originalPrice}</span>}
