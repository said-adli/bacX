    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 px-4 md:px-0">
            {items.map((stat, i) => {
                ...
                return (
                    <div key={i} className="group relative overflow-hidden rounded-3xl bg-white/5 border border-white/10 backdrop-blur-md p-6 transition-all duration-300 hover:bg-white/10 hover:border-white/20 hover:scale-[1.02] hover:shadow-2xl transform-gpu will-change-transform">

                        {/* Glow Effect */}
                        <div className={`absolute top-0 right-0 -mr-8 -mt-8 w-24 h-24 rounded-full blur-[10px] opacity-20 group-hover:opacity-40 transition-opacity ${stat.color.replace('text-', 'bg-')}`} />
